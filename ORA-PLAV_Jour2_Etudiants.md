...
## Table des matières Jour 2

| Concept | Tables TechInfo | Durée estimée |
|---|---|---|
| 1: Transactions autonomes | `TI_JOURNAL_AUDIT`, `TI_EMPLOYES` |  |
| 2: SAVEPOINT | `TI_COMMANDES`, `TI_LIGNES_CMD` |  |
| 3: DEFINER / INVOKER rights | `TI_EMPLOYES`, `TI_DEPARTEMENTS` |  |
| 4: Dictionnaire de données | `USER_OBJECTS`, `USER_SOURCE` |  |
| 5: Récursivité | `TI_EMPLOYES` (mgr_id) |  |
| 6: Fonctions PIPELINED | `TI_PRODUITS`, `TI_CATEGORIES` |  |
| 7: Surcharge et DETERMINISTIC | `TI_PARAMETRES` |  |
| 8: RESULT_CACHE | `TI_PARAMETRES` |  |
| 9: Triggers composés | `TI_EMPLOYES`, `TI_HISTORIQUE_SAL` |  |
| 10: Triggers DDL et système | `TI_JOURNAL_AUDIT` |  |
| 11: Packages Oracle intégrés | Tous |  |
| 12: SQL dynamique | `TI_EMPLOYES`, dictionnaire |  |
| Exercice intégré | Tous |  |
...
```
Transaction principale          Transaction autonome (log_audit)
─────────────────────────────   ──────────────────────────────────
BEGIN                           (attend d'être appelée)
  INSERT commande               |
  log_audit('INSERT')  ─────── > BEGIN
  ROLLBACK  ◄── annule           INSERT dans TI_JOURNAL_AUDIT
              la commande        COMMIT  ◄── persist, même si parent ROLLBACK
                                END
```
